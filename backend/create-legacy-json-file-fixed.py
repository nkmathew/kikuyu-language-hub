#!/usr/bin/env python3
"""
Create Legacy JSON File - Fixed Version

This script creates a legacy kikuyu-phrases.json file with all entries
from the curated content files. It saves the JSON in the correct format
expected by the FlashCardManager class (wrapped in a "phrases" object).

Usage:
    python create-legacy-json-file-fixed.py
"""
...
def create_legacy_json_file(project_root, all_entries):
    """Create a legacy JSON file with all entries in the correct format"""
    ...
    # Create a legacy object with a phrases array - THIS IS THE CRITICAL FIX
    legacy_object = {"phrases": legacy_entries}

    # Save as an object with a phrases array to match expected format in FlashCardManager.kt
    with open(target_file, 'w', encoding='utf-8') as f:
        json.dump(legacy_object, f, indent=2, ensure_ascii=False)
...
def main():
    """Main function"""
    print("Creating Legacy JSON File (Fixed Format)")
    print("=" * 40)
